...
@section('back')
    <a class="text-gray-700" href="{{ route('panel.apartments.index') }}">{{ __('Apartments Management') }}</a>
@endsection

<x-app-layout>
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 bg-white">
<form method="post" action="{{ route('panel.apartments.store', $address->id) }}" class="p-3 space-y-6 max-w-xl overflow-hidden shadow-sm sm:rounded-lg">
    @csrf

    <h1 class="text-lg">{{ __('Create new apartment(s)') }}</h1>
    <div>
    ...
    <div class="flex items-center gap-4">
        ...
        @if (session('status') === 'apartment-created')
            <p
                x-data="{ show: true }"
                x-show="show"
                x-transition
