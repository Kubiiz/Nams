<div class="p-3 max-w-xl bg-white overflow-hidden shadow-sm sm:rounded-lg">
    <h1 class="text-lg py-4">{{ __('Activate company') }}</h1>

    <x-alert :type="'info'">
        {{ __("This company will be activated in our system. Companies owner will receive email about activation.") }}
    </x-alert>

    <div class="pt-5">
        <x-button :type="'success'" x-data="" x-on:click.prevent="$dispatch('open-modal', 'confirm-deactivation')">
            <i class="fa fa-check text-base mr-1"></i> {{ __('Activate') }}
        </x-button>
...
<x-modal name="confirm-deactivation">
    <form method="post" action="{{ route('panel.companies.status', $result->id) }}" class="p-6">
        ...
        <h2 class="text-lg font-medium text-gray-900">
            {{ __("Are you sure you want to activate this company?") }}
        </h2>
